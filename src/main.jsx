...
function Admin({ toast, setView, refreshQuestions }) {
  ...
  async function handleCreateQuestion() {
    if (!questionForm.subject) return toast('Vui lòng chọn môn học');
    if (!questionForm.question.trim()) return toast('Vui lòng nhập nội dung câu hỏi');
    if (!questionForm.options.A?.trim() || !questionForm.options.B?.trim()) return toast('Bắt buộc có đáp án A và B');
    if (!questionForm.answer) return toast('Vui lòng nhập đáp án đúng (VD: A)');

    const result = await api('/admin/questions', { method: 'POST', body: JSON.stringify(questionForm) });
    if (!result.ok) return toast(result.data?.error || 'Không thêm được câu hỏi');

    toast('Đã thêm câu hỏi mới thành công!', 'success');
    setFilter(questionForm.subject);
    setQuestionForm({
